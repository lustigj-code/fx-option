  ...
  try {
    ...
    return parsed
      .map((entry) => ({
        ...
        name: entry.name,
        mfaVerified: Boolean(entry.mfaVerified),
      }))
...
const seedFromEnv = (): DemoUser[] => {
  const email = process.env.PORTAL_DEMO_USERNAME;
  const password = process.env.PORTAL_DEMO_PASSWORD;
  const roles = normalizeRoles(process.env.PORTAL_DEMO_ROLES);
  if (!email || !password) {
    return [];
  }
  return [
    {
      ...
      email,
      password,
      roles: roles.length > 0 ? roles : ['treasury_manager'],
...
const hydrate = () => {
  ...
  const seedPath = process.env.PORTAL_ROLE_SEED_PATH;
  if (seedPath) {
