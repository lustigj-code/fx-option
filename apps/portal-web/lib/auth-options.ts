...
import { isMfaVerified, sessionRequiresMfa, withNormalizedRoles } from '@shared/auth';
import {
  AccountLockedError,
  assertLoginAllowed,
...
const SESSION_MAX_AGE = Number.parseInt(
  process.env.PORTAL_SESSION_MAX_AGE ?? process.env.SESSION_MAX_AGE ?? '28800',
  10,
);
...
export const authOptions: NextAuthOptions = {
  providers: [
    CredentialsProvider({
      ...
      authorize: async (credentials, req) => {
        ...
        if (!email || !password) {
          if (email) {
            recordLoginFailure(email, 'PORTAL');
          }
        ...
        try {
          assertLoginAllowed(email, 'PORTAL');
        } catch (error) {
        ...
        const authenticated = await authenticateSeedUser({
          email,
          password,
          seedFile: process.env.PORTAL_AUTH_ROLE_SEED_FILE,
          envPrefix: 'PORTAL',
        });
        ...
        clearLoginFailures(email, 'PORTAL');
        return {
          id: authenticated.id,
  ...
  callbacks: {
    ...
    async signIn({ user }) {
      clearLoginFailures((user as any).email ?? user.email ?? 'unknown', 'PORTAL');
      await emitLoginSuccess({
        userId: (user as any).id ?? user.email ?? 'unknown',
        email: user.email,
        roles: Array.isArray((user as any).roles) ? ((user as any).roles as string[]) : [],
